...
/**
 * Implementation of *weightened quick union with path compression* data structure
 * Find is O(lg*(n)) --> linear time
 * Union - O(1)
 */
public class PathCompressionWeightenedQuickUnion implements DataStructure {
    private int[] elements;
    private int[] treeSizeForEachElem;
    private int MAX_DEPTH = 3;

    public PathCompressionWeightenedQuickUnion(int size) {
...
//        }
    ...
    public int root(int i) {
        int depth = 0;
        while (elements[i] != i) {
            //the closest parent root
            i = elements[i];
            //assign the root of the parent  root to the grandparent root
            //--> flatten the tree
